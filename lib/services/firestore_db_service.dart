...
class FirestoreDBServise implements DataBaseService
{
  ...
  Future<bool> saveUserWithEmailAndPassword(MyUser saveUser) async {
    ...
    await _firestoreAuth.collection("Kullanıcılar").doc(saveUser.userID).set(saveUser.toMap());
  ...
  Future<MyUser> readUser(String? id) async{
    DocumentSnapshot<Map<String, dynamic>>  _okunanUser= await _firestoreAuth.collection("Kullanıcılar").doc(id).get();

  MyUser okunanUserNesnesi;
  okunanUserNesnesi= MyUser.fromMap(_okunanUser.data()as Map<String, dynamic> );
  print("Okunan user bilgisi "+okunanUserNesnesi.toString());
  return okunanUserNesnesi;
  }

  @override
  Future<bool> updateUserAd(String UserId, String UserAd,String UserSoyad,String UserAdres) async{
  await _firestoreAuth.collection("Kullanıcılar").doc(UserId).update({"ad":UserAd,"soyad":UserSoyad,"adres":UserAdres});
  return true;
